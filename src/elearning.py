...
from google_api import get_homework_list, get_assignment_list, add_assignment, update_assignment
...
def assignment_due_soon(message):
    ...
    assignment_id = get_assignment_id(assignment_list, new_title)
...
def assignment_submitted(message):
    ...
    # upload task to google calander
    class_number, assignment_name, due_date = extract_info(message["body"]["content"])
    print(extract_info(message["body"]["content"]))
...
def extract_info(message):
    ...
    assignment_match = re.search(r'^\|\s+(?!\|)(.+)', text, re.MULTILINE)
